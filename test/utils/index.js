...
const hashMessageEIP191SolidityKeccak = (address, hash) => {
    ...
    const message = address
        ? ethers.ethers.utils.solidityKeccak256(["address", "bytes32"], [address, hash])
...
const unpackDERsignature = (signature) => {
    ...
    if (parseInt("0x" + header0) !== 0x30) {
        console.log(parseInt("0x" + header0))
...
function generateCmd(cmd, keyslot, address, hash) {
...
const getSignatureFromScan = async ({ chipAddress, address, hash, signer }) => {
    // if (!IS_BROWSER) {
    ...
    // });
    const sigString = await signer.signMessage("Hello world")
    // if (!sig) {
    ...
    // const chipAddress = ethers.ethers.utils.computeAddress("0x" + chipAddress);
    ...
    if (ethers.ethers.utils
        ...
        .toLowerCase() === chipAddress.toLowerCase()) {
        vType = "27";
    }
