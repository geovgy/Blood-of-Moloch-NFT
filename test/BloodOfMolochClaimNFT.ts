...
describe("Claim NFT", function () {
  ...
  it("Should mint to user if msg.value >= min price", async function () {
    ...
    const receipt = contract
      .connect(rando)
      .mintClaimToken({ value: parseEther("0.069") });
  ...
  it("Should revert mint if below min price", async function () {
    ...
    const receipt = contract
      .connect(rando)
      .mintClaimToken({ value: parseEther("0.01") });
    await expect(receipt).to.be.revertedWith(
      "BloodOfMolochClaimNFT: msg.value below min price"
    );
  });
  ...
  it("Should batch mint to user if msg.value >= min price", async function () {
    ...
    const receipt = contract
      .connect(rando)
      .batchMintClaimTokens(quantity, { value: parseEther("0.15") });
    for (let i = 0; i < quantity; i++) {
      await expect(receipt)
        .to.emit(contract, "Minted")
        .withArgs(i)
  ...
  it("Should revert mint if below min price", async function () {
    ...
    const receipt = contract
      .connect(rando)
      .batchMintClaimTokens(quantity, { value: parseEther("0.14") });
    await expect(receipt).to.be.revertedWith(
      "BloodOfMolochClaimNFT: msg.value below min price"
    );
  });
